...
export default function Banner() {
  ...
  const getAllBanner = async () => {
    setIsLoading(true);
    const res = await fetch("https://nationalbiomed.com.np/api/brand");
    if (res.ok) {
      const result = await res.json();
      setData(result);
      setIsLoading(false);
    } else {
      console.error("Error Fetching Product Brand");
      setIsLoading(false);
      return;
    }
  };
  ...
  return (
    <>
      <div className="flex mt-5 px-3  sm:mt-2 items-center justify-between mb-4 ">
        <p className="text-xl font-medium sm:pb-0">Brand</p>
  ...
  py-5   rounded-sm hover:opacity-85 hover:bg-textColor hover:text-white sm:space-x-3 sm:px-4   flex"
            >
              <span className="hidden md:block">Add Brand</span>
              <PlusIcon />
            </Button>
          </DialogTrigger>
          <DialogContent
